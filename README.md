...
- Works with the [Monaco Editor plugin](https://github.com/ochipin/redmine_monaco_editor) too: when it replaces a textarea, the plugin binds to the Monaco model instead (edits, remote carets and selections as Monaco decorations, undo/redo limited to your own edits), and users on Monaco co-edit with users on the plain textarea.
